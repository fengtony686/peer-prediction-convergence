    ...
    def chooseOption(self):
        if self.strategy == "FLP":
            ...
            if len(self.reports) == 0:
                ...
                totalAverage = np.sum([self.strategyProbList[i] * averageRewards[i] for i in range(self.strategyNum)])
                newStrategyProbList = []
                for i in range(self.strategyNum):
                    newStrategyProbList.append(self.strategyProbList[i] * averageRewards[i] / totalAverage)
            ...
            for i in range(self.strategyNum):
                if len(self.reports) != 0:
                    upperConfidenceBound = np.append(upperConfidenceBound, np.sqrt(np.log(len(self.reports)) / self.options.count(i)) if self.options.count(i) != 0 else np.sqrt(np.log(len(self.reports)) * 2))
                else:
                    upperConfidenceBound = np.append(upperConfidenceBound, np.random.rand())
            for index, i in enumerate(self.options):
                for j in range(self.strategyNum):
                    if i == j:
                        upperConfidenceBound[i] += self.possiblePayoffs[i][index] / self.options.count(i)
...
newGame = informationElicitationGame({(0, 0): 0.03, (0, 1): 0.01, (1, 0): 0.01, (1, 1): 0.95}, "Self Agreement")
...
print(np.sum(newGame.agents[0].possiblePayoffs[0]),np.sum(newGame.agents[0].possiblePayoffs[1]),np.sum(newGame.agents[0].possiblePayoffs[2]),np.sum(newGame.agents[0].possiblePayoffs[3]))
